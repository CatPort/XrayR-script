...
install_acme() {
    curl https://raw.githubusercontent.com/CatPort/XrayR-script/master/acme.sh | sh
}
...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/CatPort/XrayR/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/CatPort/XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/CatPort/XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    file="https://raw.githubusercontent.com/CatPort/XrayR-script/master/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Để cài đặt mới, vui lòng tham khảo hướng dẫn trước: https://catport.net, cấu hình nội dung cần thiết"
    else
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/CatPort/XrayR-script/master/XrayR.sh
