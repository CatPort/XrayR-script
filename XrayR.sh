...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/CatPort/XrayR-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/CatPort/XrayR-script/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/CatPort/XrayR-script ---
